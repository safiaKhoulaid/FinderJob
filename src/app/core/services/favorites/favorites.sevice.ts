...
export class FavoriteService {
  ...
  toggleFavorite(job: Job): Observable<{ action: string; favorite?: Favorite }> {
    const userId = this.authService.currentUser()?.id;
    if (!userId) {
    ...
    return this.http.get<Favorite[]>(`${this.apiUrl}?userId=${userId}&jobId=${job.id}`).pipe(
      switchMap(existingFavs => {
        if (existingFavs.length > 0) {
          ...
          return this.http.post<Favorite>(this.apiUrl, newFav).pipe(
            map((created) => ({ action: 'added' as string, favorite: created }))
          );
        }
      })
    );
